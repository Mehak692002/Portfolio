...
def fallback_response(message: str) -> str:
    ...
    if any(term in lower for term in ["frs", "cvvrs", "face recognition", "spoof", "motion detection", "mediapipe", "onnx", "computer vision"]):
        return (
            "CVVRS (Computer Vision Video Recognition System) is her project building components of a "
            "Python-based Face Recognition System using MediaPipe and ONNX, including spoof detection, plus a "
            "motion-detection pipeline, applying threading and concurrent-processing concepts to keep the "
            "pipeline efficient."
        )
    ...
    if any(term in lower for term in ["jira_reports", "jira reports", "jira automation", "jenkins", "reporting", "sprint activity", "scrum quality", "defect quality", "review activity"]):
        return (
            "JIRA_REPORTS Framework is her project automating Jira-based engineering report generation using "
            "Python APIs and Jenkins, with Defect Quality, Scrum Quality, Review Activity, and Sprint Activity "
            "tracking. It's open source at github.com/Mehak692002/jira_reports."
        )
    if any(term in lower for term in ["project", "sleep", "solitude"]):
        return (
            "Her notable projects include CVVRS (a Face Recognition System and motion-detection pipeline), a "
            "NATS messaging library, the JIRA_REPORTS Framework, Solitude Selections, and Sleep Pattern "
            "Analysis."
        )
